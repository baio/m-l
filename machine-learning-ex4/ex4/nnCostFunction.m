...
onehot = repmat((1:num_labels), m, 1) == repmat(y, 1, num_labels);

J = sum(sum(onehot .* (-log(h)) + (1 - onehot) .* (-log(h-1)))) / m;
...
% 1 * k
onehot = [1:num_labels] == repmat(y(i), 1, num_labels);
